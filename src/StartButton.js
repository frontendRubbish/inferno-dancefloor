...
function startPlay(props) {
  audioPlayer.querySelector('source').setAttribute('src', 'tracks/track01.mp3');
  audioPlayer.load();
  audioPlayer.play();
...
function startRiot(props) {
  audioPlayer.querySelector('source').setAttribute('src', 'tracks/track02.mp3');
  audioPlayer.load();
  audioPlayer.play();
...
const StartButton = (props) => {
    ...
    return (
      <div className={ 'start ' + ( props.dancing ? 'start--hidden' : '') }>
        <audio class="start__player" loop="true" ref={setPlayerRef}>
          <source src="" type="audio/mpeg" />
        </audio>
        <button class="start__button" onClick={ () => startPlay(props) }>
  <svg
...
<button class="start__button" onClick={  () => startRiot(props) }>
<svg
   xmlns:svg="http://www.w3.org/2000/svg"
   xmlns="http://www.w3.org/2000/svg"
   version="1.0"
...
StartButton.defaultHooks = {
  onComponentDidUpdate( lastProps, nextProps ) {
    if ( !nextProps.dancing ) {
      audioPlayer.pause();
